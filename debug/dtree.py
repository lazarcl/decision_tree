...
i decided to make a class to hold all the info for the csp so I didn't have to pass lots of 
parameters into each function
'''
import sys
import math
import random
...
def main():
    if len(sys.argv) == 2:
        ...
        (trainingSet, testSet) = create_training_set(examples, 0.06)
    elif len(sys.argv) == 3:
        ...
        print("too many arguments. Should be: python3 dtree.py trainExamples.csv testExamples.csv")
    print("creating tree")
    tree = dtree(examples, attributes, [])
    print("testing tree")
    (outcome, correctCount, totalCount)= test_tree(tree, testSet)
    print("test results: ", outcome, " percent correct.")
    print("\t", correctCount, " of total: ", totalCount)
...
#output: percentage of examples which had the correct outcome in tree
def test_tree(tree, testSet):
    ...
    for i in testSet:
        ...
        if expectedResult == treeResult:
            accurate += 1.0
        # elif treeResult != "ERROR: tree cannot handle example path":
            # print("pathError type 1")
        total += 1.0
    return ((accurate/total)*100, accurate, total)
